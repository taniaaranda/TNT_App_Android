...
class Producto(
    ...
    val foto: String,
    val tienda: String,
    val excesoSodio: Boolean,
    val excesoGrasasTot: Boolean,
    val excesoGrasasSat: Boolean,
    val excesoCalorias: Boolean,
    val excesoAzucar: Boolean,
) {
    fun getBitmapImage(): Task<ByteArray> {
        var imageRef = FirebaseStorage.getInstance()
            .getReferenceFromUrl("gs://tnt-app-android.appspot.com/camera.png")

        if (foto != "") {
            imageRef = FirebaseStorage.getInstance()
                .getReferenceFromUrl(foto)
        }

        val fiveMB: Long = 5 * 1024 * 1024
        return imageRef.getBytes(fiveMB)
    }

    fun getDefaultImage(): ByteArray {
        return Base64.decode(DEFAULT_IMAGE_BASE_64, 0)
    }

    fun incrementarStock(cantidad: Int = 1) {
        stock += cantidad
    }

    fun decrementarStock(cantidad: Int = 1) {
        if (stock > 0) {
            stock -= cantidad
        } else {
            throw Exception("El stock del producto es cero")
        }
    }
    ...
    companion object {
        ...
        fun hidratar(data: HashMap<String, Any>): Producto {
            return Producto(
                ...
                data["foto"] as String,
                data["tienda"] as String,
                data["excesoSodio"] as Boolean,
                data["excesoGrasasTot"] as Boolean,
                data["excesoGrasasSat"] as Boolean,
                data["excesoCalorias"] as Boolean,
                data["excesoAzucar"] as Boolean
            )
        }

        fun hidratar(document: DocumentSnapshot): Producto {
            return Producto(
                document.id,
                document.get("nombre").toString(),
                document.get("observaciones").toString(),
                ...
                document.get("exceso_sodio").toString().toBoolean(),
                document.get("exceso_grasas_tot").toString().toBoolean(),
                document.get("exceso_grasas_sat").toString().toBoolean(),
                document.get("exceso_calorias").toString().toBoolean(),
                document.get("exceso_azucar").toString().toBoolean()
            )
        }
    }
}